...
import {
  FileInput,
  AvCodecTypeInput,
  CameraResolutionInput,
  CameraFocusModeInput,
  CameraVideoOrientationInput,
  MicrophoneSampleRateInput,
  MicrophoneChannelInput,
  SwitchInput,
  VideoEncodeOrientationInput,
  VideoH264ProfileInput,
  BitrateAdjustModeInput,
  EncoderRCModeInput,
  CameraInput,
  NumberInput,
...
export default class Steam extends Component {
  ...
  handleAudioMixProgress = (audioMixProgress) =>
    this.setState({audioMixProgress});

  handleStreamingConfigInputChange = (text) =>
    this.setState({streamingConfigInput: text});
  handleStreamingConfigInputSubmit = () => {
    ...
    try {
      ...
      const streamingConfig = merge.recursive(
        true,
        this.state.streamingConfig,
        toMerge,
      );
  ...
  render() {
    ...
    const streamInfoText =
      streamInfo != null ? JSON.stringify(streamInfo) : 'none';
    const audioMixProgressText =
      audioMixProgress != null ? JSON.stringify(audioMixProgress) : 'none';
    const props = {
      ...streamingConfig,
        ...
        <SafeAreaView style={{display: 'flex', flex: 1}}>
          ...
          <ScrollView style={{flex: 1, backgroundColor: 'white', padding: 10}}>
            <View style={{display: 'none'}}>
              ...
              <Button
                title="提交"
                onPress={this.handleStreamingConfigInputSubmit}
              />
            </View>
            ...
            <SwitchInput
              label="手动对焦"
              {...this.bindStateOfPath('streamingConfig.focus')}
            />
            <CameraInput {...this.bindStateOfPath('streamingConfig.camera')} />

            <AvCodecTypeInput
              {...this.bindStateOfPath('streamingConfig.profile.avCodecType')}
            />

            <CameraResolutionInput
              {...this.bindStateOfPath(
                'streamingConfig.profile.cameraStreamingSetting.resolution',
              )}
            />
            <CameraFocusModeInput
              {...this.bindStateOfPath(
                'streamingConfig.profile.cameraStreamingSetting.focusMode',
              )}
            />
            <CameraVideoOrientationInput
              {...this.bindStateOfPath(
                'streamingConfig.profile.cameraStreamingSetting.videoOrientation',
              )}
            />

            <MicrophoneSampleRateInput
              {...this.bindStateOfPath(
                'streamingConfig.profile.microphoneSteamingSetting.sampleRate',
              )}
            />
            <MicrophoneChannelInput
              {...this.bindStateOfPath(
                'streamingConfig.profile.microphoneSteamingSetting.channel',
              )}
            />
            <SwitchInput
              label="回音消除"
              {...this.bindStateOfPath(
                'streamingConfig.profile.microphoneSteamingSetting.isAecEnable',
              )}
            />

            <VideoEncodeOrientationInput
              {...this.bindStateOfPath(
                'streamingConfig.profile.videoStreamingSetting.encodeOrientation',
              )}
            />
            <VideoH264ProfileInput
              {...this.bindStateOfPath(
                'streamingConfig.profile.videoStreamingSetting.h264Profile',
              )}
            />
            ...
            <NumberInput
              label="自定义视频编码分辨率宽度"
              {...this.bindStateOfPath(
                'streamingConfig.profile.videoStreamingSetting.customVideoEncodeSize.width',
              )}
            />
            <NumberInput
              label="自定义视频编码分辨率高度"
              {...this.bindStateOfPath(
                'streamingConfig.profile.videoStreamingSetting.customVideoEncodeSize.height',
              )}
            />
            ...
            <NumberInput
              label="音频采样率"
              {...this.bindStateOfPath(
                'streamingConfig.profile.audioStreamingSetting.rate',
              )}
            />
            <NumberInput
              label="音频码率"
              {...this.bindStateOfPath(
                'streamingConfig.profile.audioStreamingSetting.bitrate',
              )}
            />

            <SwitchInput
              label="使用 QUIC 协议"
              {...this.bindStateOfPath('streamingConfig.profile.quicEnable')}
            />
            <BitrateAdjustModeInput
              {...this.bindStateOfPath(
                'streamingConfig.profile.bitrateAdjustMode',
              )}
            />
            <NumberInput
              label="自适应码率调节范围下限"
            ...
            <NumberInput
              label="自适应码率调节范围上限"
              {...this.bindStateOfPath(
                'streamingConfig.profile.adaptiveBitrateRange.maxBitrate',
              )}
            />

            <EncoderRCModeInput
              {...this.bindStateOfPath('streamingConfig.profile.encoderRCMode')}
            />

            <NumberInput
              label="streamInfo 更新间隔"
              {...this.bindStateOfPath(
                'streamingConfig.profile.streamInfoUpdateInterval',
              )}
            />

            <SwitchInput
              label="内置美颜"
              {...this.bindStateOfPath('streamingConfig.faceBeautyEnable')}
            />
            <NumberInput
              label="内置美颜磨皮程度"
              {...this.bindStateOfPath(
                'streamingConfig.faceBeautySetting.beautyLevel',
              )}
            />
            <NumberInput
              label="内置美颜美白程度"
              {...this.bindStateOfPath(
                'streamingConfig.faceBeautySetting.whiten',
              )}
            />
            <NumberInput
              label="内置美颜红润程度"
              {...this.bindStateOfPath(
                'streamingConfig.faceBeautySetting.redden',
              )}
            />

            <FileInput
              label="水印文件"
              {...this.bindStateOfPath('streamingConfig.watermarkSetting.src')}
              initialFromUrl="http://oyojsr1f8.bkt.clouddn.com/qiniu_logo.png"
            />
            <NumberInput
              label="水印文件透明度"
              {...this.bindStateOfPath(
                'streamingConfig.watermarkSetting.alpha',
              )}
            />
            <NumberInput
              label="水印水平位置"
              {...this.bindStateOfPath(
                'streamingConfig.watermarkSetting.position.x',
              )}
            />
            <NumberInput
              label="水印垂直位置"
              {...this.bindStateOfPath(
                'streamingConfig.watermarkSetting.position.y',
              )}
            />
            <NumberInput
              label="水印宽度"
              {...this.bindStateOfPath(
                'streamingConfig.watermarkSetting.size.width',
              )}
            />
            <NumberInput
              label="水印高度"
              {...this.bindStateOfPath(
                'streamingConfig.watermarkSetting.size.height',
              )}
            />
            ...
            <SwitchInput
              label="图片推流"
              {...this.bindStateOfPath(
                'streamingConfig.pictureStreamingEnable',
              )}
            />
            <FileInput
              label="图片推流文件"
              {...this.bindStateOfPath('streamingConfig.pictureStreamingFile')}
              initialFromUrl="http://oyojsr1f8.bkt.clouddn.com/pause_publish.png"
            />

            <SwitchInput
              label="开启闪光灯"
              {...this.bindStateOfPath('streamingConfig.torchEnable')}
            />
            <SwitchInput
              label="预览镜像设置"
              {...this.bindStateOfPath('streamingConfig.previewMirrorEnable')}
            />
            <SwitchInput
              label="编码镜像设置"
              {...this.bindStateOfPath('streamingConfig.encodingMirrorEnable')}
            />

            <SwitchInput
              label="播放混音文件"
              {...this.bindStateOfPath('streamingConfig.playMixAudio')}
            />
            <FileInput
              label="混音文件"
              {...this.bindStateOfPath('streamingConfig.audioMixFile.filePath')}
              initialFromUrl="http://oyojsr1f8.bkt.clouddn.com/Lovestoned-Thursdays.mp3"
            />
            <SwitchInput
              label="混音文件循环播放"
              {...this.bindStateOfPath('streamingConfig.audioMixFile.loop')}
            />
            <NumberInput
              label="混音音量 micVolume"
              {...this.bindStateOfPath(
                'streamingConfig.audioMixVolume.micVolume',
              )}
            />
            <NumberInput
              label="混音音量 musicVolume"
              {...this.bindStateOfPath(
                'streamingConfig.audioMixVolume.musicVolume',
              )}
            />

            <SwitchInput
              label="返听功能"
              {...this.bindStateOfPath('streamingConfig.playbackEnable')}
            />
